...
namespace {
    constexpr char licenseKey[] = "AXjyHKX/////AAABmSE5vWFIJ06vht9qs8TG0BlJsp8al/ABvQE"
                                  "";
...
void AppController::initAR(const InitConfig &initConfig, int target) {
    /**
     * NOTE: inicializa uma instância da câmara e o Vuforia, carrega os trackers
     * necessários e carrega a base de dados com as imagens a detetar
     */
...
bool AppController::startAR() {
    ...
    /**
     * NOTE: inicia a câmara, a renderização e os trackers
     */
    ...
    if (!Vuforia::CameraDevice::getInstance().setFocusMode(
...
void AppController::cameraPerformAutoFocus() {
    Vuforia::CameraDevice::getInstance().setFocusMode(
            Vuforia::CameraDevice::FOCUS_MODE_TRIGGERAUTO);
}
...
void AppController::updateRenderingPrimitives() {
    mCurrentRenderingPrimitives.reset(new Vuforia::RenderingPrimitives(
...
bool AppController::prepareToRender(double *viewport, Vuforia::RenderData *renderData,
                                    Vuforia::TextureUnit *videoBackgroundTextureUnit,
                                    Vuforia::TextureData *videoBackgroundTexture) {
    ...
    //return renderer.updateVideoBackgroundTexture(nullptr);
}
...
bool AppController::getOrigin(Vuforia::Matrix44F &projectionMatrix,
    ...
    if (origin != nullptr) {
        if (origin->getStatus() == Vuforia::TrackableResult::STATUS::TRACKED &&
            ...
            Vuforia::Matrix44F viewMatrix = Vuforia::Tool::convertPose2GLMatrix(
                    origin->getPose());
            modelViewMatrix = MathUtils::Matrix44FTranspose(
                    MathUtils::Matrix44FInverse(viewMatrix));

            projectionMatrix = Vuforia::Tool::convertPerspectiveProjection2GLMatrix(
                    mCurrentRenderingPrimitives->getProjectionMatrix(
...
const char *AppController::getImageTargetResult(Vuforia::Matrix44F &projectionMatrix,
                                                Vuforia::Matrix44F &modelViewMatrix,
                                                Vuforia::Matrix44F &scaledModelViewMatrix) {

    /**
     * NOTE: Método alterado para não devolver apenas um booleano referente à deteção
     * de uma imagem. Devolve agora uma string com o imagem detetada
     */
    ...
    for (const auto *result : trackableResultList) {
        ...
        if (result->isOfType(Vuforia::ImageTargetResult::getClassType()) &&
            ...
            Vuforia::Matrix44F viewMatrix = Vuforia::Tool::convertPose2GLMatrix(
                    mVuforiaState.getDeviceTrackableResult()->getPose());
            viewMatrix = MathUtils::Matrix44FTranspose(
                    MathUtils::Matrix44FInverse(viewMatrix));
            ...
            scaledModelViewMatrix = MathUtils::Matrix44FScale(targetSize,
                                                              modelViewMatrix);
    ...
    for (const auto *result : trackableResultList) {
        ...
        if (result->isOfType(Vuforia::ModelTargetResult::getClassType()) &&
            ...
            if (mtResult->getStatus() == Vuforia::TrackableResult::NO_POSE) {
                if (mtResult->getStatusInfo() ==
                    Vuforia::TrackableResult::NO_DETECTION_RECOMMENDING_GUIDANCE) {
                ...
                Vuforia::Vec3F translateCenter = Vuforia::Vec3F(
                        boundingBox.getCenter().data[0], boundingBox.getCenter().data[1],
                        boundingBox.getCenter().data[2]);
                ...
                MathUtils::multiplyMatrix(translateMatrix, scaleMatrix,
                                          scaledModelViewMatrix);
                MathUtils::multiplyMatrix(modelViewMatrix, scaledModelViewMatrix,
                                          scaledModelViewMatrix);
...
bool AppController::getModelTargetGuideView(Vuforia::Matrix44F &projectionMatrix,
    ...
    if (guideViewList.size() != 0) {
        ...
        if (cameraCalibration != nullptr) {
            ...
            float guideViewAspectRatio =
                    (float) guideImage->getWidth() / guideImage->getHeight();
            ...
            if (guideViewAspectRatio >= 1.0f &&
                mDisplayAspectRatio >= 1.0f) // guideview landscape, camera landscape
            {
            ...
            } else if (guideViewAspectRatio < 1.0f &&
                       mDisplayAspectRatio < 1.0f) // guideview portrait, camera portrait
            {
            ...
            scale = Vuforia::Vec2F(2 * planeWidth / nearPlaneWidth,
                                   2 * planeHeight / nearPlaneHeight);

            modelViewMatrix = MathUtils::Matrix44FScale(
                    Vuforia::Vec3F(scale.data[0], scale.data[1], 1.0f), modelViewMatrix);
...
bool AppController::loadTrackerData() {
    ...
    if (mTarget == IMAGE_TARGET_ID) {
        //NOTE: carregamento das bases de dados de imagens
        mCurrentDataSet = loadAndActivateDataSet("VuforiaMuseumImages.xml");
        if (mCurrentDataSet == nullptr) {
...
bool AppController::unloadTrackerData() {
    ...
    Vuforia::TrackerManager &trackerManager = Vuforia::TrackerManager::getInstance();
    Vuforia::ObjectTracker *objectTracker = static_cast<Vuforia::ObjectTracker *>(trackerManager.getTracker(
            Vuforia::ObjectTracker::getClassType()));
...
bool AppController::startTrackers() {
    Vuforia::TrackerManager &trackerManager = Vuforia::TrackerManager::getInstance();
    Vuforia::Tracker *deviceTracker = trackerManager.getTracker(
            Vuforia::PositionalDeviceTracker::getClassType());
    if (deviceTracker != 0) {
        deviceTracker->start();
    }
    Vuforia::Tracker *tracker = trackerManager.getTracker(
            Vuforia::ObjectTracker::getClassType());
    if (tracker == 0) {
        return false;
    }
...
void AppController::stopTrackers() {
    ...
    Vuforia::Tracker *objectTracker = trackerManager.getTracker(
            Vuforia::ObjectTracker::getClassType());
...
void AppController::configureVideoBackground(float viewWidth, float viewHeight) {
    ...
    if (isScreenPortrait()) {
        ...
        if (aspectRatioVideo < aspectRatioView) {
            ...
            config.mSize.data[0] = int(
                    videoMode.mHeight * (viewHeight / float(videoMode.mWidth)));
            ...
            config.mSize.data[1] = int(
                    videoMode.mWidth * (viewWidth / float(videoMode.mHeight)));
        }
        ...
        if (aspectRatioVideo < aspectRatioView) {
            ...
            config.mSize.data[0] = int(viewWidth);
            config.mSize.data[1] = int(
                    videoMode.mHeight * (viewWidth / float(videoMode.mWidth)));
        } else {
            ...
            config.mSize.data[0] = int(
                    videoMode.mWidth * (viewHeight / float(videoMode.mHeight)));
            config.mSize.data[1] = int(viewHeight);
        }
